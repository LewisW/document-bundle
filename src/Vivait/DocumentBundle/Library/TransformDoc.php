...
use Vivait\DocumentBundle\Exception\UnableToGeneratePDFException;
use XSLTProcessor;
use ZipArchive;
...
class TransformDoc
{
     ...
     * @access public
     */
    public function __construct($XSLPath = null)
    {
        $this->XSLPath = $XSLPath ?: realpath(__DIR__ . '/../../xsl');
    }
    ...
    public function generateXHTML()
    {
        ...
        if (!$package->open($this->strFile)) {
            throw new InvalidDocumentException('Unable to find the DOCX file');
        }
        ...
        foreach ($relations->Relationship as $rel) {
            if ($rel["Type"] == TransformDoc::SCHEMA_OFFICEDOCUMENT) {
                ...
                @$xmlDOM->loadXML($xml);
                $xsl = new DOMDocument();
                $xsl->load($this->getXSLPath() . DIRECTORY_SEPARATOR . 'docx2html.xsl');
    ...
    public function generatePDF($path = '')
    {
        ...
        try {
            $domPDF = new Dompdf();
            $domPDF->loadHtml($this->_xhtml);
            $docProps = $this->getDocProps();
            $domPDF->setPaper($docProps['size'], $docProps['orientation']);
            $domPDF->render();
            ...
            throw new UnableToGeneratePDFException('Unable to generate PDF file.');
        }
    }
